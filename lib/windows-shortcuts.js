...
exports.query = function (path, callback) {
	execFile(__dirname + '/shortcut/Shortcut.exe',
		...
		function (error, stdout, stderr) {
			// console.log("Executing Shortcut.exe with path:", path);

			if (error) {
				console.error('Error executing Shortcut.exe:', error);
				console.error('stderr:', stderr);
				callback(stderr || stdout.toString(), null);
				return;
			}

			const iconv = require('iconv-lite');
			const decodedOutput = iconv.decode(Buffer.from(stdout), 'gbk');
			// console.log('Decoded stdout:', decodedOutput);

			var result = parseQuery(decodedOutput);
